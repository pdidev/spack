...
class Flowvr(CMakePackage):
    ...
    version('2.3.2', sha256='4543bd2a9e52f363c981a2aafacae4bf5ae7dd96d7757ebf60b809334b4f3932')
    version('2.3.1', sha256='57cdd18363ef8956c317d63e601fb8667015f3480a8a32fedc1370031aebc278')
    version('2.3.0', sha256='eef405c77eacd9eda86383d84a16dd12805b2f8c35c10560813e635fd20dc692')
    
    variant('mpi-plugin', default=True,  description='Enable FlowVR-Daemon MPI plugin.')
    variant('contrib',    default=False, description='Enable Flowvr Contribs (various components, API extensions, tests, utility tools, etc.)')
    variant('debug',      default=False, description='Enable -DDEBUG flag')
    variant('doc',        default=False, description='Enable Doxygen doc for FlowVR')
    ...
    variant('python',     default=True,  description='Build FlowVR Python Module API')
    ...
    def cmake_args(self):
        return [
            '-DBUILD_FLOWVRD_MPI_PLUGIN:BOOL={:s}'.format('ON' if '+mpi-plugin' in self.spec else 'OFF'),
            '-DBUILD_FLOWVR_CONTRIB:BOOL={:s}'.format('ON' if '+contrib' in self.spec else 'OFF'),
            '-DBUILD_FLOWVR_DEBUG_MESSAGES:BOOL={:s}'.format('ON' if '+debug' in self.spec else 'OFF'),
            '-DBUILD_FLOWVR_DOXYGEN:BOOL={:s}'.format('ON' if '+doc' in self.spec else 'OFF'),
            '-DBUILD_FLOWVR_GLGRAPH:BOOL={:s}'.format('ON' if '+glgraph' in self.spec else 'OFF'),
            '-DBUILD_FLOWVR_GLTRACE:BOOL={:s}'.format('ON' if '+gltrace' in self.spec else 'OFF'),
            '-DBUILD_FLOWVR_PYTHONMODULEAPI:BOOL={:s}'.format('ON' if '+python' in self.spec else 'OFF'),
